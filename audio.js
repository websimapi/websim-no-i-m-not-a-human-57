...
let mumbleBuffer = null, mumbleHandle = null;
...
export async function loadAllSounds(){
  ...
  mumbleBuffer = await loadSound('mumble_voice.mp3');
}
...
export function startMumbleVoice(volume=0.6){
  if(!audioUnlocked || !mumbleBuffer) return;
  mumbleHandle = playSound(mumbleBuffer, volume);
}
export function stopMumbleVoice(fadeOut=0.4){
  const h = mumbleHandle; mumbleHandle = null; if(!h) return;
  try { h.gainNode.gain.linearRampToValueAtTime(0, audioCtx.currentTime+fadeOut);
        setTimeout(()=>{ try{ h.source.stop(); }catch{} }, fadeOut*1000); } catch {}
}