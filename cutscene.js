...
import { audioCtx, getBackgroundAudio, playGateCreak, startGateLongCreak, stopGateLongCreak, playGateThud, playGateStuck, playGateFrameClank, startMumbleVoice, stopMumbleVoice } from './audio.js';
...
const scenes = [
  { // Scene 1: Landscape with birds
    ...
    onStart: (cs, canvas) => {
      ...
      scene1MumbleTimer = setTimeout(()=> startMumbleVoice(0.55), 3000);
      animateBirds(() => {
    ...
    onEnd: () => {
      if (scene1MumbleTimer) { clearTimeout(scene1MumbleTimer); scene1MumbleTimer = null; }
      stopMumbleVoice(0.3);
      stopBirds(false);
    }
  },
